...
func Create[Ti any, To any](c *gin.Context, caller func(ctx context.Context, req *Ti) (*To, error)) {
	...
	if err != nil {
		c.AbortWithStatusJSON(http.StatusBadRequest,
			helper.GenerateBaseResponseWithValidationError(false, err, "Error", "Internal error"))
		return
	}
	...
	if err != nil {
		c.AbortWithStatusJSON(helper.TranslateErrorToStatusCode(err),
			helper.GenerateBaseResponseWithError(false, err, "Error", "Internal error"))
		return
	}
...
func Delete(c *gin.Context, caller func(ctx context.Context, id int) error) {
	...
	if id == 0 {
		c.AbortWithStatusJSON(http.StatusNotFound,
			helper.GenerateBaseResponse(nil, false, "Error", "Internal error"))
		return
	}
